...
- **v1.1.0 (current)** added three CLI capabilities: a **`qork help`** command + man pages (`help`/`update`/`uninstall` are whole-word, case-insensitive commands that never get mistaken for URLs; a real URL with a scheme or dotted host always routes to the shortener; `man qork` works on `.deb`/`.rpm`/`.pkg`); a **pre-shorten liveness check** that rejects accidentally-pasted text offline and refuses a live 404/410 or an unresolvable host (auth walls 401/403, 5xx, and slow sites still shorten), skippable with **`--no-check`**; and **full origin-aware uninstall** (`qork uninstall [--yes]`) on every platform incl. Windows — it detects how qork was installed (Global/Corporate MSI via Windows Installer, Inno EXE via its uninstaller, cargo/script via self-delete) and removes the binary, PATH entry, Add/Remove-Programs entry, and install marker (`--yes`/`-y` skips the confirmation, required for scripts). `qork update` was already install-method-aware (unchanged).
- **Distribution (qork v1.1.0, GitHub Releases + crates.io)**: every release ships a native-installer matrix at `github.com/QubeTX/qork/releases/latest/download/` — **4 Windows** (Global MSI per-machine, Corporate MSI per-user/no-admin, plus Global + Corporate Inno `setup.exe`; `qork update` re-runs the matching installer via an `HKCU\Software\Qork\InstallSource` marker), **2 macOS** `.pkg` (Apple Silicon + Intel, unsigned → right-click → Open on first run), and **4 Linux** `.deb`/`.rpm` (x86-64 + ARM). Three workflows build them: cargo-dist `release.yml` (archives + shell/PS installers + Global MSI), `windows-installers.yml` (Corporate MSI + Inno EXEs), `unix-installers.yml` (`.pkg` + `.deb`/`.rpm` via `nfpm`). Recommended install: the one-liner / `cargo install qork` on macOS/Linux, the MSI/EXE on Windows.
