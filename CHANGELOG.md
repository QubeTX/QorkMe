...
- **The inline Windows installer block on `/install` now has padding** — the `GLOBAL`/`CORPORATE` rows and `↓ MSI` / `↓ EXE` buttons are inset to align with the command/notes above and gain bottom spacing, instead of hugging the panel edges (the divider stays full-bleed). (`qorkme/app/install/install.module.css`.)
- **The `QORK // SAMPLE SESSION` terminal demo now holds ~1.4s after each printed result before the next command types in** — a real-terminal beat between output and the next input, added to the `TerminalFrame` boot-print cadence as a hold on every output→input (`accent`→`prompt`) transition (reduced motion unaffected). (`qorkme/components/terminal/TerminalFrame.tsx`.)
- **The `QORK // API` panel separates its `# GET` and `# POST` examples with a blank line** for readability. (`qorkme/app/install/page.tsx`.)
...
- **QR Gen Footer Link** — Added "QR Gen" link to `https://qr.qork.me` in the site footer, positioned left of the Admin button. Opens in a new tab.

### Changed

- **Admin Button Dimmed** — Reduced Admin link opacity to 45% so the new QR Gen link is more visually prominent. Admin hover restores full visibility.

### Files Modified

- `qorkme/components/SiteFooter.tsx` — Added QR Gen link and dimmed Admin in both mobile and desktop footer layouts

## 2026-03-13 (Mobile Performance Optimization)

### Changed

- **Mobile Entrance Animations Disabled** — Removed all initial page load animations on mobile (<768px) to eliminate lag on phones. Matrix dot cascade (684 individual CSS animations), URL shortener card fade-in, and footer fade-in now render instantly on mobile.
- **AmbientDecor Hidden on Mobile** — GPU-composited blur orbs and floating particles are no longer rendered on mobile viewports, reducing GPU memory and battery usage.

### Unchanged

- All desktop entrance animations (cascade, staggered fade-ins) remain intact.
- All post-load interactive animations (hover effects, shimmer, grid ripple, card tilt, clock updates) preserved on both mobile and desktop.

### Files Modified

- `qorkme/components/MatrixDisplay.tsx` — Set `cascadeDelay={0}` on mobile Matrix instances
- `qorkme/app/globals.css` — Added `@media (max-width: 767px)` override for `.animate-fadeIn-delay-*` classes
- `qorkme/components/ui/ambient-decor.tsx` — Added `hidden md:block` to container
...
- **Clickable Qork Logo** — All Qork logo instances across the site now link back to the homepage (`/`):
  - Homepage hero logo
  - 404 page logo
  - Admin dashboard logo
  - Site footer logo (mobile and desktop)
  - Site header logo (already linked)
- **Footer Subtitle** — Changed default footer text from "Thoughtful short links" to "A Service By QubeTX." across all pages. Removed per-page custom subtitles (admin, login, result) in favor of the unified default.
- **Terracotta Logo Color** — Hero logos (homepage, 404, admin) and header logo now retain the native terracotta `rgb(191,103,71)` in dark mode instead of inverting to white. Footer logo stays white in dark mode but reveals terracotta on hover.
