...
export function createCanvasRenderer(
    ...
    (time) => {
      ...
      for (let y = 0; y < rows; y++)
        for (let x = 0; x < columns; x++) {
          ...
          const ey = (v - 0.5) * 2;
          const edge =
            Math.max(0, 1 - Math.pow(Math.abs(ex), 4)) *
            (page ? 0.85 : Math.max(0, 1 - Math.pow(Math.abs(ey), 4)));
          const central = page
            ? 1
            : 0.25 + Math.min(1, Math.abs(ex) * 1.7 + Math.abs(ey) * 0.85) * 0.75;
          const wave =
            Math.sin(
              u * 12 + Math.sin(v * 7 + time * 0.22) * 1.5 + time * 0.2 + vx[index] * 0.018
            ) *
              0.5 +
            0.5;
          const signal =
          ...
          const alpha =
            edge *
            central *
            (0.08 + signal * 0.5 + influence * 0.25 + pulse * 0.25) *
            (page ? 0.72 : 1.18);
  ...
  const resize = () => {
    ...
    width = box.width;
    height = box.height;
    if (page) columns = Math.max(1, Math.min(180, Math.ceil(width / 11)));
    ...
    const dpr = Math.min(
      devicePixelRatio || 1,
      page ? 1.25 : 1.5,
      page ? Math.sqrt(2_000_000 / Math.max(1, width * height)) : Infinity
    );
